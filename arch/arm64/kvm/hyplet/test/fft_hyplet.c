...
/*
 * In this example we show how we speed up
  * a CPU intensive routine by wrapping it 
  * in a hyplet.
  * We also show that we can pass the stack as-is.
*/
...
long benchmark(float *data, float *output)
{
	int i;
...
/*
long x1,x2,x3,x4;
int test(long _x1,long _x2,long _x3,long _x4)
{
	x1 = _x1;
	x2 = _x2;
	x3 = _x3;
...
*/
int hyplet_perf_setup(void *user_hyplet,int func_id)
{
	int rc;
...
void main(int argc,char *argv[]) 
{
  ...
  if (argc <= 1) {
	printf("Must provide iterations\n");
	return;
  }

  iters = atoi(argv[1]);

  if ( hyplet_perf_setup(benchmark, func_id)){
	printf("failed to assign hyplet-rpc\n");
	return ;
  }

  gettimeofday(&t1, NULL);
  hyplet_rpc_call(func_id, data, output);
  gettimeofday(&t2, NULL);
  us = (t2.tv_sec - t1.tv_sec)/1000000 + (t2.tv_usec - t1.tv_usec);
  printf("Hyp: Duration is: %ld us for %d iterations\n", us, iters);
